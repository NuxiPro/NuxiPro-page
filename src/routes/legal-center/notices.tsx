        ...
        <div className="space-y-12">
          ...
          <section id="publisher" className="space-y-8 scroll-mt-24">
            ...
            <LegalSection title={t("legal.notices.publisher.title")}>
              <div className="space-y-2">
                <InfoRow label={t("legal.notices.publisher.name.label")} value={t("legal.notices.publisher.name.value")} />
                <InfoRow
          ...
          <section id="hosting" className="space-y-8 scroll-mt-24">
            ...
            <LegalSection title={t("legal.notices.hosting.title")}>
              <div className="space-y-2">
                <InfoRow label={t("legal.notices.hosting.provider.label")} value={t("legal.notices.hosting.provider.value")} />
                <InfoRow label={t("legal.notices.hosting.address.label")} value={t("legal.notices.hosting.address.value")} />
                <InfoRow
        ...
        <nav className="sticky top-24 rounded-2xl border border-hairline bg-white/60 backdrop-blur p-5">
          ...
          <ul className="space-y-1 text-[13px] leading-none">
            <li>
              ...
              >
                <span className={`h-1.5 w-1.5 rounded-full shrink-0 transition-colors ${active === "publisher" ? "bg-teal" : "bg-teal/50"}`} />
                {t("legal.notices.publisher.title")}
              </a>
            </li>
            <li>
              ...
              >
                <span className={`h-1.5 w-1.5 rounded-full shrink-0 transition-colors ${active === "hosting" ? "bg-teal" : "bg-teal/50"}`} />
                {t("legal.notices.hosting.title")}
              </a>
            </li>
          </ul>
...
function InfoRow({ label, value, href }: { label: string; value: string; href?: string }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
      ...
      {href ? (
        <a href={href} target="_blank" rel="noopener noreferrer" className="text-[14px] text-teal hover:text-teal-hover transition-colors">
          {value}
        </a>
      ) : (
